from typing import List

CTS = ["0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a24d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4241d3f3585f44440ad011078381bacc075e4c3", 
"0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a34b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab907f18fcf347dd433bcf83432d0849e80c22b0",  
"", 
"", 
"", 
"",  
"", 
"", 
"14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a902590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3", 
"", 
"", 
"", 
"0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944" ]

# convert hex strings to list of ints
def hex_to_bytes(s: str) -> List[int]:
    return [int(s[i:i+2], 16) for i in range(0, len(s), 2)]

def is_alpha_byte(x: int) -> bool:
    return (65 <= x <= 90) or (97 <= x <= 122)

def is_printable(x: int) -> bool:
    return 32 <= x <= 126

def detect_spaces(cts_bytes: List[List[int]], threshold: int = 7):
    #Heuristic: for each ciphertext i and position k,
    #count how many other ciphertexts j produce alphabetic (A-Z or a-z)
    #when ct[i][k] ^ ct[j][k] is computed.
    #If count >= threshold --> we assume plaintext[i][k] is a space.
    n = len(cts_bytes)
    max_len = max(len(c) for c in cts_bytes)
    space_pos = [set() for _ in range(n)]

    for i in range(n):
        for k in range(len(cts_bytes[i])):
            cnt = 0
            for j in range(n):
                if j == i or k >= len(cts_bytes[j]):
                    continue
                x = cts_bytes[i][k] ^ cts_bytes[j][k]
                if is_alpha_byte(x):
                    cnt += 1
            if cnt >= threshold:
                space_pos[i].add(k)
    return space_pos

def derive_key_from_spaces(cts_bytes: List[List[int]], space_pos: List[set]):
    #Given guessed spaces for some ciphertexts, derive key bytes:
    #key[k] = ct[i][k] ^ ord(' ') for any ciphertext i with guessed space at k.
    #If multiple i give same k, ensure consistency.
    #Return partial key dict mapping index -> byte or None.
    key = {}  # pos -> key_byte
    for i, posset in enumerate(space_pos):
        for k in posset:
            kb = cts_bytes[i][k] ^ ord(' ')
            if k in key and key[k] != kb:
                continue
            key[k] = kb
    return key

def recover_plaintexts(cts_bytes: List[List[int]], key: dict):

    #Using partial key, recover plaintext characters where possible.
    #Return list of strings (with '_' for unknown).
    recovered = []
    for c in cts_bytes:
        chars = []
        for k in range(len(c)):
            if k in key:
                p = c[k] ^ key[k]
                if is_printable(p):
                    chars.append(chr(p))
                else:
                    chars.append('?')
            else:
                chars.append('_')
        recovered.append(''.join(chars))
    return recovered

def main():
    cts_bytes = [hex_to_bytes(s) for s in CTS]
    space_pos = detect_spaces(cts_bytes, threshold=7)
    key = derive_key_from_spaces(cts_bytes, space_pos)
    recovered = recover_plaintexts(cts_bytes, key)

    for idx, pt in enumerate(recovered, 1):
        print(f"{idx:2d}: {pt}")
    
if __name__ == "__main__":
    main()
